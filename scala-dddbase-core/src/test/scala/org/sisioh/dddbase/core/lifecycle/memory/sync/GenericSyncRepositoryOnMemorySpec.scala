  ...
  "The repository" should {
    ...
    "have stored entities" in {
      ...
      for (i <- 0 to 9) yield {
        ...
        repository.resolveBy(entities(i).identity).isFailure must_== true
    ...
    "resolve a entities by using identity" in {
      ...
      repository.multiResolveBy(entities.map(_.identity): _*).isSuccess must_== true
      val _entities = repos.flatMap(_.result.multiResolveBy(entities.map(_.identity): _*)).get
    ...
    "resolve a entities by using identity" in {
      ...
      repository.multiResolveBy((0 to 9 by 2).map(Identity(_)).toSeq: _*).isSuccess must_== true
      repos.get.result.multiResolveBy((0 to 9).map(Identity(_)).toSeq: _*).isSuccess must_== true
      val _entities = repos.flatMap(_.result.multiResolveBy(entities.map(_.identity): _*)).get
      _entities must_== entities
    }
    ...
    "delete a entities by using identity" in {
      ...
      val resultWithEntity2 = resultWithEntity.flatMap {
        _.result.multiDeleteBy(entities.map(_.identity): _*)
      }.get
      (resultWithEntity2.result ne repository) must beTrue
