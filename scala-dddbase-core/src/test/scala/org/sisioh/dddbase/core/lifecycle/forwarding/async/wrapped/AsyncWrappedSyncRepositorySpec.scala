...
class AsyncWrappedSyncRepositorySpec extends Specification with Mockito {
  ...
  (protected val delegate: GenericSyncRepositoryOnMemory[Identifier[UUID], EntityImpl])
  (implicit val executor: ExecutionContext)
    extends AsyncWrappedSyncRepository[Identifier[UUID], EntityImpl] {

    type This = ForwardingAsyncWrappedRepositoryImpl

    type Delegate = GenericSyncRepositoryOnMemory[Identifier[UUID], EntityImpl]
  ...
  "The repository" should {
    "have stored entity with empty identifier" in {
      val repository = new ForwardingAsyncWrappedRepositoryImpl(GenericSyncRepositoryOnMemory[Identifier[UUID], EntityImpl]())
