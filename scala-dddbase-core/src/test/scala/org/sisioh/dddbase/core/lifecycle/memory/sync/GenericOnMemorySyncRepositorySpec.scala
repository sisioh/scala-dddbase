...
class GenericOnMemorySyncRepositorySpec extends Specification with Mockito {

  class EntityImpl(val identity: Identity[UUID])
    extends Entity[Identity[UUID]]
    with EntityCloneable[Identity[UUID], EntityImpl]
    with Ordered[EntityImpl] {
    def compare(that: GenericOnMemorySyncRepositorySpec.this.type#EntityImpl): Int = {
  ...
  "The repository" should {
    ...
    "resolveOption a entity by using identity" in {
      class TestSyncRepository extends GenericOnMemorySyncRepository[Identity[UUID], EntityImpl]
      with OnMemorySyncRepositorySupportByOption[TestSyncRepository, Identity[UUID], EntityImpl]
