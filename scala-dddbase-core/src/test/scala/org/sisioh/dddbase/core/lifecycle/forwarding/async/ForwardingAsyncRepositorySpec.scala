...
import org.sisioh.dddbase.core.lifecycle.memory.async.GenericOnMemoryAsyncRepository
...
class ForwardingAsyncRepositorySpec extends Specification with Mockito {
  ...
  (implicit _executor: ExecutionContext)
    extends ForwardingRepositoryAsync[TestRepForwardingRepositoryImplAsync, Identity[UUID], EntityImpl] {
  ...
  "repository" should {
    ...
    "not resolve a entity by using a non-existent identity" in {
      val repository = new TestRepForwardingRepositoryImplAsync(new GenericOnMemoryAsyncRepository[Identity[UUID], EntityImpl]())
