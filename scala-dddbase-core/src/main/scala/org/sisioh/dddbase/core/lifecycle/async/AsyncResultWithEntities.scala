...
object AsyncResultWithEntities {
  ...
  def apply[R <: AsyncEntityWriter[ID, T], ID <: Identifier[_], T <: Entity[ID]]
...
case class AsyncResultWithEntitiesImpl[+R <: AsyncEntityWriter[ID, T], ID <: Identifier[_], T <: Entity[ID]]
(result: R, entities: Seq[T])
