 ...
 * @tparam E エンティティの型
 */
trait AsyncEntityReader[CTX <: EntityIOContext[Future], ID <: Identity[_], E <: Entity[ID]]
  extends AsyncEntityIO with EntityReader[CTX, ID, E, Future] {
   ...
   */
  def resolve(identity: ID)(implicit ctx: CTX): Future[E]

  def resolves(identities: Seq[ID])(implicit ctx: CTX): Future[Seq[E]] = {
  ...
  def containsByIdentities(identities: Seq[ID])(implicit ctx: CTX): Future[Boolean] = {
    implicit val executor = getExecutionContext(ctx)
    Future.traverse(identities){
      identity =>
